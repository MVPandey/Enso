...
8. [Iterative Inference: Thinking at Inference Time](#8-iterative-inference-thinking-at-inference-time)
   ...
   - [SVGD: Stein Variational Gradient Descent](#svgd-stein-variational-gradient-descent)
...
## 8. Iterative Inference: Thinking at Inference Time

### The Core Idea

**File:** `src/ebm/model/jepa.py`
...
Two methods are supported. **Langevin dynamics** (the default) runs independent chains with gradient descent + noise. **SVGD** replaces independent chains with interacting particles that cooperatively explore via kernel-mediated forces. Both share the same energy computation; only the update rule differs.
...
**Files:** `src/ebm/model/kernels.py`, `src/ebm/model/jepa.py`

Langevin dynamics treats each chain independently — they explore the energy landscape in parallel but never communicate. SVGD (Liu & Wang, 2016) replaces independent chains with **interacting particles** that cooperatively explore via a kernel-mediated update rule:

$$\mathbf{z}_i \leftarrow \mathbf{z}_i - \eta \, \hat{\phi}^*(\mathbf{z}_i), \quad \hat{\phi}^*(\mathbf{z}_i) = \frac{1}{N} \sum_{j=1}^{N} \left[ k(\mathbf{z}_j, \mathbf{z}_i) \, \nabla_{\mathbf{z}_j} \log p(\mathbf{z}_j) + \nabla_{\mathbf{z}_j} k(\mathbf{z}_j, \mathbf{z}_i) \right]$$

The update has two terms:
- **Attraction** ($k \cdot \nabla \log p$): Each particle is pulled toward low-energy regions, weighted by how similar it is to other particles. Since $\log p(\mathbf{z}) \propto -E(\mathbf{z})$, the attraction term is $K \cdot (-\nabla E)$.
- **Repulsion** ($\nabla k$): Particles are pushed apart to maintain diversity. This replaces Langevin noise — instead of random exploration, diversity is maintained deterministically.

The kernel is a radial basis function (RBF):

$$k(\mathbf{z}_i, \mathbf{z}_j) = \exp\!\left(-\frac{\|\mathbf{z}_i - \mathbf{z}_j\|^2}{2h}\right)$$

where bandwidth $h$ is set via the **median heuristic**: $h = \text{median}(\|\mathbf{z}_i - \mathbf{z}_j\|^2) \,/\, (2 \log(N+1))$, computed per-puzzle on GPU.

**Critical implementation detail:** The kernel must be computed **per puzzle**, not across puzzles. The flat `z` tensor of shape $(B \cdot N, D)$ is reshaped to $(B, N, D)$ before kernel computation, so particles from different puzzles never interact.

```python
# In _svgd_step:
z_batched = z.reshape(batch_size, n_particles, d)
grad_batched = grad_z.reshape(batch_size, n_particles, d)
K, grad_K = rbf_kernel(z_batched, bandwidth)
update = svgd_update(grad_batched, K, grad_K)  # (attraction + repulsion) / N
z_new = z_batched - lr * update
```

**Why SVGD might outperform Langevin:** With independent chains, each explores blindly — two chains can waste compute by converging to the same local minimum. SVGD's repulsion term ensures particles spread across distinct energy basins, potentially discovering more diverse solutions. The hypothesis is that SVGD's accuracy advantage grows with the number of particles, since more particles amplify the cooperative exploration effect.
...
src/ebm/
    ...
    evaluation/
        solver.py           # Batch iterative inference (Langevin / SVGD)
...
**`src/ebm/utils/config.py`** — Three Pydantic configuration classes. `Config` reads API keys from `.env`. `ArchitectureConfig` holds model hyperparameters (d_model, n_layers, etc.). `TrainingConfig` holds training hyperparameters (batch size, LR, loss weights, Langevin parameters, inference method selection). All have sensible defaults for the Run 5 configuration.
...
**`src/ebm/model/kernels.py`** — RBF kernel computation and SVGD update logic. `rbf_kernel` computes the kernel matrix and its gradients for batched particles, using the median heuristic for bandwidth when not specified. `svgd_update` combines kernel-weighted attraction (negative energy gradients) and repulsion (kernel gradients) into the SVGD optimal transport direction.

**`src/ebm/model/jepa.py`** — The central orchestrator. `SudokuJEPA.__init__` wires together context encoder, target encoder, z_encoder, predictor, decoder. `forward()` is the training pass: encode puzzle and solution, add noise to z, predict, decode, return all outputs. `solve()` is iterative inference: initialize random z chains/particles, compute energy gradients, then dispatch to `_langevin_step()` (gradient descent + noise) or `_svgd_step()` (kernel-mediated update). Selects the lowest-energy chain and decodes to an integer grid.
...
**`src/ebm/evaluation/plotting.py`** — Plotting utilities for inference method comparison experiments. Provides `plot_accuracy_vs_chains`, `plot_timing`, `plot_constraint_satisfaction`, and `plot_bandwidth_sweep` — all producing publication-ready PNGs from a list of result dicts.

**`src/ebm/main.py`** — CLI entry point with `train` and `eval` subcommands. Orchestrates dataset loading, splitting, model creation, training/evaluation, and device management. The `eval` subcommand accepts `--inference-method` (langevin or svgd) and `--kernel-bandwidth` flags.
...
**`scripts/compare_inference.py`** — Experiment script that sweeps method (Langevin vs SVGD) across chain counts, with an optional bandwidth sensitivity sweep for SVGD. Outputs JSON results and comparison plots to `results/svgd_comparison/`.
...
**$T = 50$ steps** — Inference iteration count (both Langevin and SVGD). Empirically, accuracy improvements plateau after $\sim$30-50 steps. More steps add latency without meaningful gains; fewer steps don't allow sufficient convergence from random initialization. At 50 steps, the solver adds +1.0% puzzle accuracy over the forward pass.

**$K = 8$ chains/particles** — Parallel trajectories per puzzle. For Langevin, multiple chains hedge against local minima. For SVGD, more particles amplify the cooperative exploration effect — the kernel repulsion prevents particles from collapsing to the same basin. The improvement from 1 to 4 is significant; from 4 to 8 is modest; from 8 to 16 is marginal for Langevin but may be more pronounced for SVGD (the core experimental hypothesis).

**$\eta_{\text{inference}} = 0.01$** — Step size (shared by both methods). An order of magnitude smaller than the training LR ($6 \times 10^{-4}$), appropriate because inference optimization must be conservative — overshooting in latent space can produce solutions that are worse than the starting point.

**$\sigma_{\text{Langevin}} = 0.005$** — Base noise magnitude for Langevin dynamics (not used by SVGD). Half the step size, following the Langevin MCMC convention that noise should be smaller than the gradient step. Further annealed by the temperature schedule $\tau_t$, vanishing to zero by the final step. SVGD does not inject noise — the kernel repulsion term maintains diversity deterministically.

**Kernel bandwidth** $h$ **(SVGD only)** — Defaults to the median heuristic $h = \text{median}(\|\mathbf{z}\_i - \mathbf{z}\_j\|^2) / (2 \log(N+1))$, computed per-puzzle per-step on GPU. Can be overridden via `--kernel-bandwidth` for sensitivity analysis. Too small: particles barely interact (reduces to independent gradient descent). Too large: uniform repulsion regardless of distance (particles spread without regard to energy landscape).
